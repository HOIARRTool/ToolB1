...
### 7. ชุดข้อมูลสรุปเพื่อทำแผน (Planning Inputs)
ให้สรุปเป็นรายการสั้นๆ (bullet) โดยต้องมีอย่างน้อย:
- Problem statement (ประโยคเดียว ว่าปัญหาคืออะไร)
- Risk level (Low/Medium/High) + เหตุผล
- Harm / Impact ที่เกิดขึ้นหรือเกือบเกิด (กับผู้ป่วย/บุคลากร/ระบบ)
...
### 7. แผนพัฒนาที่นำไปปฏิบัติได้ทันที (Action Plan + SMART KPI)
- สรุป "Potential Change" 3-5 ข้อ ที่ต่อยอดจากข้อ 4-6 (ไม่ทวนข้อความเดิมซ้ำยาว ๆ)
- สำหรับแต่ละข้อ ให้เขียน:
  - เป้าหมาย (Goal)
  - กิจกรรม/มาตรการ (Key Actions) 2-4 ข้อ
  - เจ้าภาพ/ทีม (Owner)
  - ระยะเวลา (Timeline)
  - SMART KPI 1-2 ตัว (ระบุสูตร/นิยามตัวชี้วัดให้ชัด)

### 8. บทสรุปผู้บริหาร (Executive Summary)
...
ข้อกำหนดเพิ่มเติม:
- หลีกเลี่ยงความซ้ำซ้อนกับหัวข้อ 1-6 (อ้างอิงกลับได้ แต่ไม่คัดลอกซ้ำ)
- หากข้อมูลไม่พอ ให้ระบุ "ข้อมูลที่ควรถามเพิ่ม" แบบสั้น ๆ ในท้ายหัวข้อ 7 เท่านั้น (ไม
่ต้องถามกลับผู้ใช้)
"""
